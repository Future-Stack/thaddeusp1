...
function MyVouchers() {
  ...
  const {
    ...
    isError,
    refetch,
  } = useMyVouchers(currentPage, ITEMS_PER_PAGE);
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-10">
          <div>
            ...
            <p className="text-gray-500 mt-2 font-medium">
              Manage and redeem your won prizes
            </p>
          </div>
      ...
      {isLoading ? (
        ...
        <div className="bg-red-50 border border-red-100 rounded-3xl p-12 text-center max-w-2xl mx-auto shadow-2xl shadow-red-100/20 mt-10">
          ...
          <p className="text-red-700/80 mb-8 font-medium">
            We couldn't load your vouchers. Please try again.
          </p>
          <button
            onClick={() => refetch()}
        ...
        <div className="bg-white border border-gray-100 rounded-4xl p-16 text-center max-w-3xl mx-auto shadow-2xl shadow-gray-100/50 mt-10">
          ...
          <h2 className="text-2xl md:text-3xl font-black text-[#1A202C] mb-4">
            No Vouchers Yet
          </h2>
          <p className="text-gray-500 mb-10 text-lg font-medium">
            Keep playing to win amazing prizes and see your vouchers here!
          </p>
        </div>
      ) : (
        <>
  ...
  return (
    ...
    >
      ...
      <div className="bg-[#FF5100] p-7 text-white relative overflow-hidden">
        <div className="absolute -right-4 -top-4 text-9xl opacity-10 rotate-12 transition-transform group-hover:rotate-45 duration-500">
          🎉
        </div>
        <div className="relative z-10">
          ...
          <h3 className="text-2xl font-black truncate leading-tight">
            {voucher.draw?.event?.name || "Free Prize"}
          </h3>
          <p className="text-sm font-medium opacity-90 mt-1">
            ${voucher.value} Reward Value
          </p>
        </div>
      </div>
      ...
      <div className="p-7 grow space-y-6">
        <div className="flex items-center justify-between">
          <div
            className={`px-4 py-1.5 rounded-full text-[10px] font-black tracking-wider ${statusColors[voucher.status as keyof typeof statusColors]}`}
          >
            {voucher.status}
          </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-4 text-gray-600">
            ...
            <div>
              ...
              <p className="text-[15px] font-black text-[#1A202C]">
                {format(
                  new Date(voucher.draw?.drawnAt || voucher.createdAt),
                  "MMM dd, yyyy",
                )}
              </p>
            </div>
