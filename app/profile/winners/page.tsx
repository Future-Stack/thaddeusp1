...
const WinnersPage = () => {
  ...
  return (
    <div className="min-h-screen selection:bg-[#FF5100]/10 ">
      {/* Hero Section */}
      <section
        className="py-9.25 md:px-8 relative overflow-hidden rounded-[10px]"
        style={{
        ...
        <div className="max-w-6xl mx-auto relative z-10">
          <div className="text-center">
            ...
            {isLoading ? (
              ...
              <AnimationWrapper animationType="scaleUp" delay={0.2}>
                <div
                  className="max-w-2xl mx-auto px-8 py-5.5 text-white border border-white/20 "
                  style={{
                    borderRadius: "10px",
                    background: "rgba(255, 255, 255, 0.10)",
                  }}
                >
                  <h2 className="text-xl md:text-[30px] font-bold mb-1 leading-none">
                    {thisWeeksWinner.name}
                  </h2>
                  <p className="text-white text-lg md:text-xl mb-4">
                    {thisWeeksWinner.address}
                  </p>

                  <div className="w-full h-px bg-white/50 mb-4 mx-auto"></div>

                  <div className="space-y-3">
                    <p className="text-xl md:text-[18px] tracking-tight">
                      {thisWeeksWinner.eventName} - Prize:{" "}
                      {thisWeeksWinner.prize}
                    </p>
                    <p className="text-white text-sm">
                      Draw Date:{" "}
                      {format(
                        new Date(thisWeeksWinner.winDate),
                        "MMMM dd, yyyy",
                      )}
                    </p>
                  </div>
              ...
              <div className="text-white opacity-60">
                No winner announced yet this week.
              </div>
            )}
          </div>
        </div>
        ...
        <div className="mt-4">
          <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-6">
            <AnimationWrapper animationType="fadeRight">
              <h2 className="text-2xl md:text-[32px] font-bold text-[#1A202C] tracking-tight">
                Past Winners
              </h2>
            </AnimationWrapper>
          </div>
            ...
            {isLoading ? (
              Array(8)
                ...
                .map((_, i) => (
                  <div
                    key={i}
                    className="bg-white rounded-[14px] p-4 flex items-center gap-4 border border-gray-100"
                  >
              ...
              winners.map((winner, i) => (
                ...
                >
                  <div
                    className="bg-white rounded-[14px] p-4 flex items-center gap-4 group cursor-pointer relative overflow-hidden"
                    style={{
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-start mb-0.5">
                        <h4 className="font-black text-[#1A202C] truncate text-[15px] group-hover:text-[#FF5100] transition-colors">
                          {winner.name}
                        </h4>
                      ...
                      <p className="text-[11px] font-bold text-gray-400 truncate tracking-wide">
                        {winner.address}
                      </p>
                    </div>
                  </div>
          ...
          {meta && meta.totalPages > 1 && (
            <div className="mt-16 flex flex-col sm:flex-row items-center justify-between gap-6 pt-12 border-t border-gray-50">
              <p className="text-sm font-bold text-gray-400">
                ...
                of <span className="text-[#1A202C]">{meta.total}</span> winners
              </p>
